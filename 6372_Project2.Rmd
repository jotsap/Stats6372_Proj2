...
```{r}
library(ggplot2)
ggplot(data = full_wdbc, aes(x = Comp.1, y = Comp.2)) +
  geom_point(aes(col=pc_wdbc_c), size=1)+
  ggtitle("PCA of Breast Cancer Tumor Biopsies")

ggplot(data = full_wdbc, aes(x = Comp.2, y = Comp.3)) +
  geom_point(aes(col=pc_wdbc_c), size=1)+
  ggtitle("PCA of Breast Cancer Tumor Biopsies")
...
main.glm <- glm(diag_bool ~ . , data=bc.boolean, family = binomial(link = "logit") , control = list(maxit = 50))
summary(main.glm)
...
```{r}
# REDUCED model removing all "SE"" measurements, all "Worst"", and only using "Area" in place of 'perimeter' and 'radius'
redux.glm <- glm(diag_bool ~ texture_mean + area_mean + smoothness_mean + compactness_mean + concavity_mean + concave_points_mean + symmetry_mean + fractal_dimension_mean , data=bc.boolean, family = binomial(link = "logit") , control = list(maxit = 60))
summary(redux.glm)
